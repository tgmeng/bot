...
const [token, ...tocPaths] = process.argv.slice(2);
...
async function main() {
  ...
  let tocPathIndex = 0;
  let reportRoot;
  while (tocPaths[tocPathIndex]) {
    reportRoot = tocTree.find(
      item => item.data.title === tocPaths[tocPathIndex]
    )[0];
    if (!reportRoot) {
      console.error(`找不到 "${tocPaths.join(">")}", 不整理 TOC 了`);
    }
    tocPathIndex++;
  }
